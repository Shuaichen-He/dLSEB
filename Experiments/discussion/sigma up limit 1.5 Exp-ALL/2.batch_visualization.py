...
For each (experiment, site) combination, generates:
  1. losses.svg  — loss & parameter evolution across epochs
  2. model_output.svg — energy-balance time-series (obs vs EST vs optimised)
...
# =============================================================================
def plot_loss_multi(save_path, exp_name, site_name, T_loss, params_hat):
...
def plot_energy_balance(save_path, obs, opt_data, est_data, k_opt, k_est,
                        site_name="Huazhaizi"):
    ...
    plt.savefig(save_path, format="svg", dpi=300)
    plt.close(fig)
    print(f"    [✓] model_output.svg saved")
...
def run_visualization(site_name, exp_mode, plot_loss=True, plot_energy=True,
    ...
    if plot_loss:
        plot_loss_multi(
            os.path.join(site_out_dir, "losses.svg"),
            exp_mode, site_name,
    ...
    if plot_energy:
        plot_energy_balance(
            os.path.join(site_out_dir, "model_output.svg"),
