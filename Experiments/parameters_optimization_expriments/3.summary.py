...
# =============================================================================
def plot_grouped_bars(data_dict, ylabels, ylims, titles, out_dir, stem,
    ...
    plt.savefig(os.path.join(svg_dir, f"{stem}.svg"), format="svg", dpi=300, bbox_inches="tight")
    print(f"  [✓] svg/{stem}.svg saved")
    plt.savefig(os.path.join(pdf_dir, f"{stem}.pdf"), format="pdf", dpi=300, bbox_inches="tight")
    print(f"  [✓] pdf/{stem}.pdf saved")
    plt.close(fig)
...
# =============================================================================
def plot_grouped_lines(data_dict, ylabels, ylims, titles, out_dir, stem,
                       data_label="RMSE", hline_y=None, ylim_cap=None,
                       legend_headroom=0.40):
...
if __name__ == "__main__":
    ...
    plot_fn(
        ...
        SCRIPT_DIR, f"rmse_{chart_kind}", data_label="RMSE",
    )

    # ── Plot Correlation ──
    plot_fn(
        COR, COR_YLABELS, COR_YLIMS, cor_titles,
        SCRIPT_DIR, f"cor_{chart_kind}", data_label="Correlation",
    ...
    if args.compute_std_bias:
        ...
        plot_fn(
            ...
            SCRIPT_DIR, f"std_{chart_kind}", data_label="Std Ratio",
        )
        plot_fn(
            bias, BIAS_YLABELS, BIAS_YLIMS, bias_titles,
            SCRIPT_DIR, f"bias_{chart_kind}", data_label="Bias",
